...
def export(data=None, source=PROCESSED_DIR, write=True) -> dict:
    ...
    orders=pd.read_csv(source/"SalesOrderHeader.csv")
    ...
    po_headers=pd.read_csv(source/"PurchaseOrderHeader.csv",parse_dates=["CreatedDate"])
    receipts=pd.read_csv(source/"PurchaseReceipt.csv",parse_dates=["ReceiptDate"]).merge(po_headers[["PONumber","VendorID","CreatedDate"]],on="PONumber")
    ...
    booking_region=order_headers.merge(pd.read_csv(source/"Customer.csv")[["CustomerID","Region"]],on="CustomerID",how="left")
    ...
    rep_summary=sales_product.groupby("SalesRepID",as_index=False).agg(Revenue=("Revenue","sum"),GrossProfit=("GrossProfit","sum"),Invoices=("InvoiceID","nunique")).merge(pd.read_csv(source/"SalesRep.csv")[["SalesRepID","SalesRepName","Region"]],on="SalesRepID",how="left")
    ...
    dq_path=source/"data_quality.csv"
...
if __name__=="__main__": raise SystemExit("Use python -m etl.run_pipeline --skip-generation")
