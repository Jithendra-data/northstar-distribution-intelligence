...
function renderExecutiveSignals(data){
 ...
 if(k.open_po_value>0)add(60,'Purchasing commitments',money(f.overdue_po_value),'Overdue unreceived purchase value; open the largest overdue commitments.','purchasing');
 ...
 section.innerHTML=`<div class="metric-heading"><h2 id="signals-title">Executive Signals</h2><span>Full dataset · refreshed with each published run</span></div><div class="signal-list">${selected.map((s,i)=>`<a class="signal-card" data-focus="${h(s.title==='Supplier lead time'?supplier?.VendorID||'':s.title==='Warehouse service'?warehouses[0]?.[0]||'':'')}" href="#${s.target}"><span class="signal-rank">0${i+1}</span><div><small>${h(s.title)}</small><strong>${h(s.value)}</strong><p>${h(s.detail)}</p><span class="signal-action">${h(({inventory:"Inventory Planning · Review disposition and replenishment holds; resolve when positions have an approved action.",purchasing:"Procurement · Review vendor commitments; resolve when dates or quantities are confirmed.",customers:"Sales Operations · Review inactive accounts; resolve with an agreed outreach disposition.",operations:"Warehouse Operations · Review the service gap and investigate its causes.",sales:"Finance · Investigate margin drivers before proposing a pricing change."})[s.target])}</span></div><span aria-hidden="true">↗</span></a>`).join('')||'<p>No observations available in this dataset.</p>'}</div><details class="signal-method"><summary>How signals are prioritized</summary><p>Rule-based triage: inventory exposure first, followed by worsening supplier lead time, inactive high-value customers, service gaps, purchasing commitments, and margin movement. Larger supplier delays, service gaps, or margin declines raise priority. The five highest scores are shown. These are investigation prompts, not forecasts or business targets. Overview filters do not change these full-dataset signals.</p></details>`;
