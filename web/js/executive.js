...
function renderLineage(data){
 ...
 old.replaceWith(box);
 const select=i=>{box.querySelectorAll('button').forEach((b,j)=>b.setAttribute('aria-pressed',String(i===j)));const s=stages[i];box.querySelector('#lineage-detail').innerHTML=`<span>STAGE 0${i+1} / 07</span><h3>${s[0]} · ${s[1]}</h3><p>${h(s[2])}</p><a href="https://github.com/Jithendra-data/northstar-distribution-intelligence/tree/main/${s[3]}" target="_blank" rel="noreferrer">View implementation ↗</a>`};
 box.querySelectorAll('button').forEach((button,i)=>{button.addEventListener('click',()=>select(i));button.addEventListener('keydown',event=>{const next=event.key==='ArrowRight'?(i+1)%7:event.key==='ArrowLeft'?(i+6)%7:event.key==='Home'?0:event.key==='End'?6:null;if(next!==null){event.preventDefault();box.querySelectorAll('button')[next].focus();select(next)}})});select(0);
}
